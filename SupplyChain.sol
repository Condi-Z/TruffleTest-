...
contract supplyChain {
    ...
    function getParticipant(uint32 _product_id) public view returns (string memory, address, string memory) {
        return (participants[_product_id].userName,
                participants[_product_id].participantAdderss,
    ...
    function addProduct(uint32 _ownerId, 
        ...
        if(keccak256(abi.encodePacked(participants[_ownerId].participantType)) == keccak256("Manufacturer")) {
            ...
            products[productId].productOwner = participants[_ownerId].participantAdderss;
    ...
    function newOwner(uint32 _user1Id, uint32 _user2Id, uint32 _prodId) onlyOwner(_prodId) public returns (bool) {
        ...
        if(keccak256(abi.encodePacked(p1.participantType)) == keccak256("Manufacturer")
           ...
           ownerships[ownership_id].productOwner = p2.participantAdderss;
           ownerships[ownership_id].ownerId = _user2Id;
           ownerships[ownership_id].trxTimeStamp = uint32(block.timestamp);
           products[_prodId].productOwner = p2.participantAdderss;
    ...
    function aythenticateParticipant(uint32 _uid,
